...
export default function ProfileView() {
  const { 
    ...
    tradingGoalImage, setTradingGoalImage
  } = useAppContext();
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const fileName = `vision_${Math.random().toString(36).substring(2, 15)}_${Date.now()}.${ext}`;
    
    setLoading(true);
    const { error, data } = await supabase.storage.from('trade_images').upload(fileName, file, {
       cacheControl: '3600',
       upsert: false,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('profiles').upsert({
      ...
      trading_goal_image: tradingGoalImage
    });
    setLoading(false);
    if (error) alert(error.message);
    else {
      // Small tactile feedback trick (just wait 500ms)
    ...
    <div className="px-4 pb-24 w-full max-w-lg mx-auto relative animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <form onSubmit={handleSave} className="flex flex-col gap-6">
        ...
        <div className="glass-panel p-5 ring-1 ring-white/5 border-none bg-gradient-to-br from-[var(--dondo-emerald)]/5 to-transparent flex flex-col gap-4">
           <h3 className="text-[10px] text-[var(--dondo-emerald)] font-bold tracking-widest uppercase text-center">The Goal</h3>
           ...
           <div>
              ...
              <textarea 
                 ...
                 className="w-full h-24 bg-black/60 border border-[var(--dondo-emerald)]/20 rounded-xl p-4 text-white text-xs font-medium leading-relaxed outline-none focus:border-[var(--dondo-emerald)] resize-none transition shadow-inner" 
              />
           </div>

           <div>
              <label className="text-[10px] text-white/50 font-bold uppercase tracking-widest mb-1.5 block">Vision Board Image</label>
              
              <div className="relative overflow-hidden w-full h-32 rounded-xl border border-[var(--dondo-emerald)]/20 shadow-[0_0_20px_rgba(16,185,129,0.1)] group flex items-center justify-center bg-black/60 cursor-pointer transition hover:border-[var(--dondo-emerald)]">
                 {tradingGoalImage ? (
                    <img src={tradingGoalImage} className="absolute inset-0 w-full h-full object-cover z-0" />
                 ) : (
                    <span className="text-[var(--dondo-emerald)] font-black text-xs uppercase tracking-widest z-0">+ UPLOAD VISION IMAGE</span>
                 )}
                 ...
                 {tradingGoalImage && (
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity z-10 flex items-center justify-center backdrop-blur-sm">
                       <span className="text-white font-black text-[10px] uppercase tracking-widest">TAP TO REPLACE</span>
                    </div>
                 )}

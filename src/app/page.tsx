...
export default function Home() {
  ...
  // Prevent "Ghost Login" by waiting for auth to fully load
  if (loadingAuth) {
    return (
  ...
  // If no user is authenticated, force them to the AuthPage
  if (!user) {
    return <AuthPage />;
  }
  ...
  return (
    <div className="bg-black min-h-screen w-full relative overflow-x-hidden">
       ...
       {/* Top Dashboard Action Buttons - Only visible on Dashboard */}
       {activeTab === "DASHBOARD" && (
         <div className="px-5 mb-8 flex gap-3 animate-in fade-in slide-in-from-top-2 duration-500">
            <button 
              onClick={() => setIsTradeModalOpen(true)} 
              className="flex-1 py-4 bg-[var(--dondo-emerald)] text-black font-black uppercase tracking-widest rounded-2xl flex items-center justify-center gap-2 text-xs shadow-[0_10px_20px_rgba(16,185,129,0.2)] hover:opacity-90 active:scale-95 transition-all"
            >
               <Plus size={18} className="stroke-[3]" /> LOG TRADE
            </button>
            <button 
              onClick={() => setIsWithdrawalModalOpen(true)} 
              className="px-6 bg-[#fbbf24] text-black font-black rounded-2xl flex items-center justify-center shadow-[0_10px_20px_rgba(251,191,36,0.2)] hover:opacity-90 active:scale-95 transition-all"
            >
               <Wallet size={18} />
            </button>
         </div>
       ...
       <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 z-10 relative pb-40">
         ...
         {/* Global Footer - Inside scroll area */}
